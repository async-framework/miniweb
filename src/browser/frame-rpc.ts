  ...
  return {
    call(command, payload) {
      ...
      return new Promise((resolve, reject) => {
        const timer = window.setTimeout(() => {
          ...
          reject(new Error(`MiniWeb frame RPC timed out: ${command}`));
        }, timeoutMs);
    ...
    destroy() {
      ...
      for (const entry of pending.values()) {
        window.clearTimeout(entry.timer);
        entry.reject(new Error('MiniWeb frame RPC client destroyed'));
      }
      pending.clear();
    }
  };
...
export function createFrameRpcServer(options: {
  ...
  async function onMessage(event: MessageEvent): Promise<void> {
    ...
    try {
      if (!handler) {
        throw new Error(`Unknown MiniWeb frame RPC command: ${event.data.command}`);
      }
