...
import {
  ...
  redirect,
  toApp
} from '../core/create-miniweb-router.ts';
import type {
  FetchApp,
  MiniWeb,
...
export async function bootBrowserShell(root: Document = document, initialPath = '/'): Promise<MiniWeb> {
...
function formatTraceEntries(entries: MiniWebTraceEntry[]): string {
  const lines: string[] = [];
...
async function createWeb(mode: RuntimeMode): Promise<MiniWeb> {
  return createMiniWeb(browserShellMiniWeb, runtimeOptions(mode));
}

function runtimeOptions(mode: RuntimeMode): MiniWebCreateOptions {
...
async function renderFrame(
  ...
  } else if (response.headers.get('x-miniweb-stream') === '1' && response.body) {
...
async function renderHtmlFrame(
  ...
  if (response.headers.get('x-miniweb-stream') !== '1' || !response.body) {
...
function dispatchFrameStreamUpdate(frame: HTMLIFrameElement, detail: FrameStreamUpdateDetail): void {
  ...
  frameDocument.dispatchEvent(new EventConstructor('miniweb:stream-chunk', {
    detail
  }));
}
...
export const exampleDirectoryCards: ExampleDirectoryCard[] = [
  {
    ...
    summary: 'The original manifest app served through MiniWeb.',
    setup: {
  ...
  },
  {
    ...
    setup: {
      ...
      behavior: 'browser code fetches backend routes through MiniWeb'
    }
  },
  {
  ...
  },
  {
    ...
    setup: {
      ...
      behavior: 'MiniWeb dispatches across registered app origins'
    }
  },
  {
    ...
    setup: {
      apps: 'frontend app plus backend app',
      runtime: 'backend platform fetch is scoped to MiniWeb',
  ...
  },
  {
    ...
    setup: {
      ...
      behavior: 'state is scoped to the MiniWeb app platform'
    }
  }
];
...
const browserShellMiniWeb: MiniWebAppDefinition = createMiniWebApp({
