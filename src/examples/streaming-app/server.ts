...
const streamSections: StreamSection[] = [
  {
    ...
    body: 'The frontend asks for a document, MiniWeb builds a Request, and the route graph forwards it to this browser-safe app.fetch() handler.'
  },
  {
    title: 'First Byte',
  ...
  },
  {
    ...
    body: 'This is the GitHub Pages use case: code talks to itself through MiniWeb instead of relying on a deployed API.'
  },
  {
    title: 'Runtime Swap',
...
function createHomeChunks(delay: StreamDelayProfile, search: string): string[] {
  ...
  return [
        ...
        <main>
          ...
          <script>
            (() => {
              ...
              document.addEventListener('miniweb:stream-chunk', (event) => {
                update(event.detail || {});
              });
    ...
    `     </section>
          <p class="meta">Done: MiniWeb streamed and rendered the complete long document.</p>
        </main>
      </body>
    </html>`
  ];
