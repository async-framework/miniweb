...
export type MiniWebMiddlewareInput = MiniWebMiddleware | MiniWebMiddleware[];
export type MiniWebCacheStore = 'service-worker' | 'edge' | `named:${string}`;
export type MiniWebCacheKey =
  | string
  | ((request: Request, url: URL, context: MiniWebRouteContext) => string | Promise<string>);

export interface MiniWebCacheMiddlewareOptions {
  store?: MiniWebCacheStore;
  key?: MiniWebCacheKey;
  ttl?: number;
  tags?: string[];
  query?: MiniWebCacheQueryOptions;
}

export interface MiniWebFilesMiddlewareOptions {
  publicPrefix?: string;
  filePrefix?: string;
  cacheControl?: string;
}

export function composeMiniWeb(input: MiniWebMiddlewareInput): MiniWebMiddleware {
  const middleware = Array.isArray(input) ? input : [input];

  return async function composedMiniWebMiddleware(request, context, next) {
    let index = -1;

    async function dispatch(position: number, nextRequest: Request): Promise<Response> {
      if (position <= index) {
        throw new Error('MiniWeb middleware called next() multiple times');
      }
      index = position;
...
export function domain(hostname: string, input: MiniWebMiddlewareInput): MiniWebMiddleware {
  const child = composeMiniWeb(input);
...
export function mount(prefix: string, input: MiniWebMiddlewareInput): MiniWebMiddleware {
  const child = composeMiniWeb(input);
...
export function toApp(name: string): MiniWebMiddleware {
...
export function toOrigin(baseUrl: string): MiniWebMiddleware {
  return (request) => {
...
export function toFiles(options: MiniWebFilesMiddlewareOptions = {}): MiniWebMiddleware {
...
export function cacheFirst(options: MiniWebCacheMiddlewareOptions = {}): MiniWebMiddleware {
  return async (request, context, next) => {
...
export function staleWhileRevalidate(options: MiniWebCacheMiddlewareOptions = {}): MiniWebMiddleware {
  return async (request, context, next) => {
    const cache = await resolveCacheStore(context, options);
    const cacheRequest = await createCacheRequest(request, context, options);
    const cached = await cache.match(cacheRequest, options.query);
...
export function networkFirst(options: MiniWebCacheMiddlewareOptions = {}): MiniWebMiddleware {
  return async (request, context, next) => {
    const cache = await resolveCacheStore(context, options);
    const cacheRequest = await createCacheRequest(request, context, options);
    try {
...
export function cacheOnly(options: MiniWebCacheMiddlewareOptions = {}): MiniWebMiddleware {
  return async (request, context) => {
    const cache = await resolveCacheStore(context, options);
    const cacheRequest = await createCacheRequest(request, context, options);
    const cached = await cache.match(cacheRequest, options.query);
    context.route.cacheHit = Boolean(cached);
    return cached ?? new Response('MiniWeb cache miss', {
      status: 504
    });
  };
}

export function networkOnly(): MiniWebMiddleware {
  return (_request, _context, next) => next();
}

export function redirect(url: string, status = 302): MiniWebMiddleware {
...
export function errorHandler(
  handler: (error: unknown, request: Request, context: MiniWebRouteContext) => Promise<Response> | Response
): MiniWebMiddleware {
  return async (request, context, next) => {
    try {
      return await next();
...
export function logger(): MiniWebMiddleware {
...
export function finalMiniWebHandler(request?: Request): Response {
  if (!request) {
    return new Response('Cannot handle MiniWeb request', {
      status: 500
    });
  }
  const url = new URL(request.url);
...
async function resolveCacheStore(
  ...
  options: MiniWebCacheMiddlewareOptions
): Promise<{
  store: MiniWebCacheStore;
  match(request: Request, query?: MiniWebCacheQueryOptions): Promise<Response | undefined>;
...
async function createCacheRequest(
  ...
  const keyUrl = key.startsWith('http://') || key.startsWith('https://')
    ? key
    : new URL(`/__miniweb/cache/${encodeURIComponent(key)}`, context.platform.origin).href;
...
function recordCacheTrace(
  context: MiniWebRouteContext,
  boundary: MiniWebTraceEntry['boundary'] | undefined,
  request: Request,
  response?: Response,
  detail?: unknown
): void {
...
async function withRouteState<T>(
  context: MiniWebRouteContext,
  patch: Partial<MiniWebRouteContext['route']>,
  run: () => Promise<T> | T
): Promise<T> {
  const previous = {
    ...context.route,
