...
export function createFakeLocation(initialUrl: string): FakeLocation {
  let current = parseMiniWebUrl(initialUrl, undefined);

  function set(url: string): void {
    current = parseMiniWebUrl(url, current);
  }
...
export function parseMiniWebUrl(url: string, base: URL | undefined): URL {
  try {
    return new URL(url, base);
  } catch (error) {
    throw new Error(`Invalid MiniWeb URL: ${url}`, {
      cause: error
    });
  }
}
