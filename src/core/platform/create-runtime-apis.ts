...
export function createMiniWebTimers(): MiniWebTimers {
  let nextId = 1;
...
export function createMiniWebCrypto(config: MiniWebPlatformConfig = {}): MiniWebCrypto {
  const subtle = globalThis.crypto?.subtle;
  if (!subtle) {
    throw new Error('MiniWeb platform crypto requires globalThis.crypto.subtle');
  }
...
export function createMiniWebNavigator(config: MiniWebPlatformConfig = {}): MiniWebNavigator {
  return {
    userAgent: config.navigator?.userAgent ?? 'MiniWeb/0.0.0',
    onLine: config.navigator?.onLine ?? true,
    language: config.navigator?.language ?? 'en-US'
  };
}

export function getMiniWebCustomEventConstructor(): typeof CustomEvent {
  if (typeof globalThis.CustomEvent === 'function') {
    return globalThis.CustomEvent;
  }

  return class MiniWebCustomEvent<T = unknown> extends Event implements CustomEvent<T> {
    readonly detail: T;
...
function runTimerHandler(handler: TimerHandler, arguments_: unknown[]): void {
  ...
  // Browser timers accept strings, but MiniWeb does not eval timer source.
}
