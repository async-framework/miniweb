...
export function createMiniWebCacheStorage(): MiniWebCacheStorage {
  const caches = new Map<string, MiniWebCache>();
...
function createMiniWebCache(): MiniWebCache {
  ...
  return {
    ...
    async put(request, response) {
      ...
      if (cacheRequest.method !== 'GET' && cacheRequest.method !== 'HEAD') {
        throw new TypeError('MiniWeb cache only supports GET and HEAD requests');
      }
...
function matches(
  entry: Request,
  lookup: Request,
  options: MiniWebCacheQueryOptions = {}
): boolean {
...
function cacheKey(request: Request, options: MiniWebCacheQueryOptions = {}): string {
