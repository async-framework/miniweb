...
import type { MiniWebContext } from './types.ts';
...
function installBuiltInRoutes(worker: FakeServiceWorker): void {
  ...
  workerWithRoutes.route('/__miniweb/files/', async (request, context) => {
    const url = new URL(request.url);
    const rawPath = url.pathname.slice('/__miniweb/files'.length);
    const filePath = normalizeMiniWebPath(rawPath);
