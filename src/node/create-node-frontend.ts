...
import type { MiniWeb } from '../core/types.ts';
...
export function createNodeFrontend(web: MiniWeb): NodeFrontend {
  ...
  function requireElement(selector: string): NodeElementHandle {
    ...
    if (!element) {
      throw new Error(`MiniWeb node frontend selector not found: ${selector}`);
    }
  ...
  return {
    ...
    async click(selector) {
      ...
      if (isElementTag(element, 'a')) {
        ...
        if (!href) {
          throw new Error(`MiniWeb anchor is missing href: ${selector}`);
        }
    ...
    async submit(selector, formData = {}) {
      ...
      if (!isElementTag(element, 'form')) {
        throw new Error(`MiniWeb selector is not a form: ${selector}`);
      }
