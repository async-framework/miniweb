...
import { createNodeMiniWeb } from '../src/node/create-node-miniweb.ts';
import { helloApp } from '../src/examples/hello-app/manifest.ts';

describe('node miniweb e2e', () => {
  ...
  it('submits GET and POST forms through the Node frontend', async () => {
    ...
    expect(web.frontend.text('h1')).toBe('miniweb');
    ...
    expect(web.frontend.text('h1')).toBe('name=MiniWeb');
  });
});
