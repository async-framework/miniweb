...
import type { MiniWebAppDefinition } from '../src/core/types.ts';
...
interface ExampleHomeCase {
  name: string;
  definition: MiniWebAppDefinition;
...
describe('example route coverage', () => {
  it.each(exampleHomeCases)('serves the $name root page', async ({ definition, expectedHeading }) => {
    const web = await createMiniWeb(definition);
  ...
  it('streams long streaming-app HTML content in ordered chunks', async () => {
    ...
    expect(response.status).toBe(200);
    expect(response.headers.get('x-miniweb-stream')).toBe('1');
    if (!reader) {
    ...
    expect(chunks.join('')).toContain('Done: MiniWeb streamed and rendered the complete long document.');
  });
