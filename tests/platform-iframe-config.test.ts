...
describe('MiniWeb iframe environment config', () => {
  ...
  it('creates MiniWeb-owned runtime frames with location metadata and sandbox controls', () => {
    ...
    expect(frame.dataset.miniwebLocation).toBe('https://backend.local/runtime');
  ...
  it('creates and destroys a runtime frame RPC transport without direct eval hooks', () => {
    ...
    expect(transport.frame.dataset.miniwebLocation).toBe('https://edge.local/runtime');
