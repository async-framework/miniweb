...
describe('browser miniweb', () => {
  ...
  it('boots the browser shell with an example directory and runtime presets', async () => {
    ...
    expect(frame?.srcdoc).toContain('<h1>MiniWeb Examples</h1>');
  ...
  it('labels fake browser icon buttons with delayed tooltips', async () => {
    ...
    expect(html).toContain('data-tooltip="Reset MiniWeb runtime"');
    ...
    expect(html).toContain('.miniweb-tooltip.is-visible');
  ...
  it('renders the streaming shell demo with query-configured delays', async () => {
    ...
    expect(frameText).toContain('miniweb:stream-chunk');
    expect(frameText).toContain('Done: MiniWeb streamed and rendered the complete long document.');
